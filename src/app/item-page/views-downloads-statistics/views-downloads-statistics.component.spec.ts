...
import {
  ...
  FileStatistic,
  StatsData,
  ViewsDownloadsStatisticsService,
  YearlyFileStats,
} from './views-downloads-statistics.service';
...
describe('ViewsDownloadsStatisticsComponent', () => {
  ...
  beforeEach(async () => {
    ...
    // ds-matomo-subscription-button is a real standalone import of this component on 9-base
    mockMatomoService = jasmine.createSpyObj('MatomoReportSubscriptionDataService', {
      getSubscriptionStatus: createFailedRemoteDataObject$('not subscribed', 404),
      subscribe: createFailedRemoteDataObject$('not subscribed', 404),
      unsubscribe: createFailedRemoteDataObject$('not subscribed', 404),
    });
  ...
  describe('Title and Label Generation', () => {
    it('should return yearly view title', () => {
      ...
      expect(component.getTitle()).toBe('statistics.views-downloads.title.yearly');
    });
    ...
    it('should return monthly view title with year', () => {
      ...
      expect(component.getTitle()).toBe('statistics.views-downloads.title.monthly');
    });
    ...
    it('should return all-years label when no year selected', () => {
      ...
      expect(component.getYearLabel()).toBe('statistics.views-downloads.all-years');
    });
    ...
    it('should return all-months label when year selected but no month', () => {
      ...
      expect(component.getYearLabel()).toBe('statistics.views-downloads.all-months');
    });
