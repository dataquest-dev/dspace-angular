...
describe('ClarinLicenseTableComponent', () => {
  ...
  // NOTE: the v7 original wrapped this in fakeAsync() and never tick()ed, so the whenStable()
  // callback - and every expectation inside it - never ran: the spec was vacuous. 9-base karma
  // runs jasmine with failSpecWithNoExpectations:true, which exposes that. Reading a Blob with
  // FileReader is a real browser task that neither fakeAsync nor whenStable() flushes, so the
  // spec waits for the create call itself and then asserts for real.
  it('should create new clarin license label and load table data', (done) => {
    const createSpy = clarinLicenseLabelDataService.create as jasmine.Spy;
    ...
    const assertWhenCreated = () => {
      if (!createSpy.calls.any()) {
        setTimeout(assertWhenCreated, 10);
        return;
      }
  ...
  describe('license delete button', () => {
    const getDeleteControls = () => {
      ...
      const deleteWrapper = actionsRow.query(By.css('.btn-group.pe-1:last-child span'));
  ...
  describe('label delete flow', () => {
    ...
    it('should open confirmation modal when confirmDeleteLabel is called', () => {
      ...
      // v9 ConfirmationModalComponent takes a `name` string, not a `dso`
      expect(labelDeleteModalRef.componentInstance.name).toBe(mockNonExtendedLicenseLabel.label);
    });
  ...
  describe('label row actions', () => {
    ...
    it('should disable delete button and expose tooltip for linked labels', () => {
      ...
      // Angular 20 no longer emits `ng-reflect-*` attributes, so read the bound value off the
      // NgbTooltip directive instance instead of off the DOM attribute (v7 spec did the latter).
      const linkedRowTooltip = linkedRowDeleteWrapper.injector.get(NgbTooltip);
      ...
      expect(linkedRowTooltip.ngbTooltip as string).toContain('clarin.license.label.table.del');
    });
