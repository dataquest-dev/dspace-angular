 ...
 */
export class BtnDisabledDirective implements OnDestroy {
    ...
    private readonly onKeydown = (event: Event) => this.handleKeydown(event as KeyboardEvent);

    /**
     * Listens in the capture phase. A (click) declared on the same element is coalesced into the
     * host listener, so a bubble-phase stopImmediatePropagation() can no longer reach it.
     */
