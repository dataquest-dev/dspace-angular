IF "%ENVFILE%"=="" set ENVFILE=%cd%/../run/envs/.default

:: wiki: https://wiki.lyrasis.org/display/DSDOC7x/OAI#OAI-HarvestingfromanotherDSpace
pushd ..\..
:: import community with collection
docker-compose --env-file %ENVFILE% -p dq-d7 -f docker/cli.yml -v %cd%/build-scripts/import/assets:/assets run --rm dspace-cli structure-builder -f /assets/test_community_collection.xml -o /assets/test_import_output.xml -e test@test.edu
:: test connection
